...
(function (global, factory) {
    ...
    const generatedAttribute = "generated", defaultCompositeValue = "source-over", resizeEvent = "resize", visibilityChangeEvent = "visibilitychange", percentDenominator = 100, half$1 = 0.5, millisecondsToSeconds = 1000, originPoint = {
        x: 0,
        y: 0,
        z: 0,
    }, defaultTransform = {
        ...
        c: 0,
        d: 1,
    }, randomColorValue = "random", midColorValue = "mid", double = 2, doublePI = Math.PI * double, defaultFps = 60, defaultAlpha$1 = 1, generatedTrue = "true", generatedFalse = "false", canvasTag = "canvas", defaultRetryCount = 0, squareExp$3 = 2, spatialHashGridCellSize = 100, defaultRemoveQuantity = 1, defaultRatio$1 = 1, defaultReduceFactor = 1, inverseFactorNumerator = 1, rgbMax = 255, hMax = 360, sMax = 100, lMax = 100, hMin = 0, sMin = 0, hPhase = 60, empty = 0, quarter = 0.25, threeQuarter = half$1 + quarter, minVelocity$7 = 0, defaultTransformValue = 1, minimumSize = 0, zIndexFactorOffset = 1, defaultOpacity$3 = 1, removeDeleteCount = 1, removeMinIndex = 0, defaultFpsLimit = 120, minFpsLimit = 0, canvasFirstIndex = 0, loadRandomFactor = 10000, loadMinIndex = 0, one = 1, none = 0, decayOffset = 1, tryCountIncrement = 1, minZ = 0, minLimit = 0, countOffset = 1, minCount = 0, minIndex = 0, lengthOffset = 1, defaultDensityFactor = 1, deleteCount = 1, defaultAngle = 0, identity$3 = 1, minStrokeWidth = 0, lFactor = 1, lMin = 0, triple = 3, sextuple = 6, sNormalizedOffset = 1, phaseNumerator = 1, defaultRgbMin = 0, defaultVelocity = 0, defaultLoops = 0, defaultTime = 0, defaultZoom = 1;
    ...
    class Vector3d {
        ...
        getLengthSq() {
            return this.x ** squareExp$3 + this.y ** squareExp$3;
        }
        mult(n) {
            return Vector3d.create(this.x * n, this.y * n, this.z * n);
        }
    ...
    const minRadius$6 = 0, minMemoizeSize = 0;
    ...
    function isPointInside(point, size, offset, radius, direction) {
        return areBoundsInside(calculateBounds(point, radius ?? minRadius$6), size, offset, direction);
    }
    ...
    class Circle extends BaseRange {
        ...
        intersects(range) {
            ...
            if (range instanceof Circle || range.type === RangeType.circle) {
                ...
                const rectRange = range, { width, height } = rectRange.size, edges = Math.pow(dx - width, squareExp$3) + Math.pow(dy - height, squareExp$3);
                return edges <= r ** squareExp$3 || (dx <= r + width && dy <= r + height) || dx <= width || dy <= height;
            }
            return false;
        }
        reset(x, y, radius) {
    ...
    const styleCache = new Map(), maxCacheSize = 1000, firstIndex$3 = 0, rgbFixedPrecision = 2, hslFixedPrecision = 2;
    function getCachedStyle(key, generator) {
        let cached = styleCache.get(key);
        if (!cached) {
            cached = generator();
            if (styleCache.size >= maxCacheSize) {
                const keysToDelete = [...styleCache.keys()].slice(firstIndex$3, maxCacheSize * half$1);
    ...
    class EmittersCircleShape extends EmitterShapeBase {
        ...
        randomPosition() {
            ...
            }, radius = (x, y, theta) => (x * y) / Math.sqrt((y * Math.cos(theta)) ** squareExp$2 + (x * Math.sin(theta)) ** squareExp$2), [a, b] = [size.width * half$1, size.height * half$1], randomTheta = generateTheta(a, b), maxRadius = radius(a, b, randomTheta), randomRadius = fill ? maxRadius * Math.sqrt(getRandom()) : maxRadius;
            return {
                position: {
    ...
    function randomSquareCoordinate(position, offset) {
        return position + offset * (getRandom() - half$1);
    }
    ...
    class EmittersSquareShape extends EmitterShapeBase {
        ...
        randomPosition() {
            ...
            if (fill) {
                ...
                const halfW = size.width * half$1, halfH = size.height * half$1, side = getRandomSize(sides$5), v = (getRandom() - half$1) * double;
                switch (side) {
                    case Sides.TopLeft:
                        return {
                            position: {
    ...
    class Drag {
        ...
        preserveMomentum;
        constructor() {
            this.momentumFactor = defaultMomentumFactor$1;
    ...
    function initParticle(pluginManager, particle) {
        ...
        if (rollOpt.backColor) {
            ...
            const alterType = getRandom() >= half$1 ? exports.AlterType.darken : exports.AlterType.enlighten;
            particle.roll.alter = {
                type: alterType,
    ...
    const minVelocity$6 = 0, boundsMin = 0;
    function bounceHorizontal(data) {
        ...
        let bounced = false;
        if (data.outOfCanvas &&
            ((data.direction === exports.OutModeDirection.right && velocity > minVelocity$6) ||
                (data.direction === exports.OutModeDirection.left && velocity < minVelocity$6))) {
    ...
    function bounceVertical(data) {
        ...
        let bounced = false;
        if (data.outOfCanvas &&
            ((data.direction === exports.OutModeDirection.bottom && velocity > minVelocity$6) ||
                (data.direction === exports.OutModeDirection.top && velocity < minVelocity$6))) {
        ...
        update(particle, direction, _delta, outMode) {
            ...
            switch (particle.outType) {
                ...
                case exports.ParticleOutType.inside: {
                    ...
                    if ((vx < minVelocity$5 && dx > particle.moveCenter.radius) ||
                        (vy < minVelocity$5 && dy > particle.moveCenter.radius) ||
                        (vx >= minVelocity$5 && dx < -particle.moveCenter.radius) ||
                        (vy >= minVelocity$5 && dy < -particle.moveCenter.radius)) {
                        return;
                    }
                    break;
    ...
    class NoneOutMode {
        ...
        update(particle, direction, _delta, outMode) {
            ...
            if (!gravityOptions.enable) {
                if ((particle.velocity.y > minVelocity$4 && particle.position.y <= canvasSize.height + pRadius) ||
                    (particle.velocity.y < minVelocity$4 && particle.position.y >= -pRadius) ||
                    (particle.velocity.x > minVelocity$4 && particle.position.x <= canvasSize.width + pRadius) ||
                    (particle.velocity.x < minVelocity$4 && particle.position.x >= -pRadius)) {
                    return;
                }
    ...
    const minVelocity$3 = 0, minDistance$6 = 0, updateVector$1 = Vector.origin;
    class OutOutMode {
        modes;
        #container;
        constructor(container) {
        ...
        update(particle, direction, _delta, outMode) {
            ...
            switch (particle.outType) {
                case exports.ParticleOutType.inside: {
                    ...
                    if ((vx <= minVelocity$3 && dx >= minDistance$6) ||
                        (vy <= minVelocity$3 && dy >= minDistance$6) ||
                        (vx >= minVelocity$3 && dx <= minDistance$6) ||
                        (vy >= minVelocity$3 && dy <= minDistance$6)) {
                        return;
                    }
    ...
    function drawEmoji(data, image) {
        const { context, opacity } = data, previousAlpha = context.globalAlpha, diameter = image.width, radius = diameter * half$1;
        context.globalAlpha = opacity;
    ...
    const squareExp$1 = 2, halfPI = Math.PI * half$1, toleranceFactor$1 = 10, minRadius$4 = 0, minVelocity$2 = 0;
        ...
        for (const particle of query) {
            if (area instanceof Circle) {
                circleBounce(circleBounceDataFromParticle(particle), {
                    position,
                    mass: radius ** squareExp$1 * halfPI,
    ...
    function singleSelectorBounce(container, selector, div, bounceCb) {
        ...
        query.forEach(item => {
            ...
            }, radius = elem.offsetWidth * half$1 * pxRatio, tolerance = toleranceFactor$1 * pxRatio, area = div.type === DivType.circle
    ...
    function rectSideBounce(data) {
        ...
        if ((pSide.max >= rectSide.min && pSide.max <= (rectSide.max + rectSide.min) * half$1 && velocity > minVelocity$2) ||
            (pSide.min <= rectSide.max && pSide.min > (rectSide.max + rectSide.min) * half$1 && velocity < minVelocity$2)) {
    ...
    const half = 0.5, toleranceFactor = 10, minRadius$3 = 0;
    ...
    function drawGif(data, canvasSettings) {
        ...
        const pos = { x: -image.gifData.width * half$1, y: -image.gifData.height * half$1 }, frame = image.gifData.frames[frameIndex];
    ...
    function getUnitPolygon(sides) {
        ...
        const step = doublePI / sides, isOdd = !!(sides % double), baseAngle = (-Math.PI + (isOdd ? noOffset : step)) * half$1, verts = [];
    ...
    function drawTextLine(context, line, size, index, fill, stroke) {
        const pos = {
            x: -(line.length * size * half$1),
            y: size * half$1 + index * size,
        };
        if (fill) {
    ...
    class TiltUpdater {
        ...
        init(particle) {
            ...
            particle.tilt = {
                ...
                sinDirection: getRandom() >= half$1 ? identity$3 : -identity$3,
                cosDirection: getRandom() >= half$1 ? identity$3 : -identity$3,
                min: 0,
                max: doublePI,
            };
    ...
    class Particle {
        ...
        getMass() {
            return this.getRadius() ** squareExp$3 * Math.PI * half$1;
        }
        getOpacity() {
        ...
        isShowingBack() {
            ...
            if (this.roll.horizontal) {
                const normalizedAngle = (angle + Math.PI * half$1) % (Math.PI * double), adjustedAngle = normalizedAngle < defaultAngle ? normalizedAngle + Math.PI * double : normalizedAngle;
    ...
    function applyDistance(particle) {
        ...
        if ((hasHDistance || hasVDistance) && !particle.misplaced) {
            ...
            if (hDistance &&
                ((pos.x < initialPosition.x && vel.x < minVelocity$1) || (pos.x > initialPosition.x && vel.x > minVelocity$1))) {
                vel.x *= -getRandom();
            }
            if (vDistance &&
                ((pos.y < initialPosition.y && vel.y < minVelocity$1) || (pos.y > initialPosition.y && vel.y > minVelocity$1))) {
                vel.y *= -getRandom();
            }
        }
    }
    function move(particle, moveOptions, moveSpeed, maxSpeed, moveDrift, reduceFactor, delta) {
        ...
        if (gravityOptions?.enable &&
            maxSpeed > minVelocity$1 &&
            ((!gravityOptions.inverse && velocity.y >= minVelocity$1 && velocity.y >= maxSpeed) ||
                (gravityOptions.inverse && velocity.y <= minVelocity$1 && velocity.y <= -maxSpeed))) {
    ...
    function initSpin(container, particle) {
        ...
        particle.spin = {
            center: spinCenter,
            direction: particle.velocity.x >= minVelocity$1 ? exports.RotateDirection.clockwise : exports.RotateDirection.counterClockwise,
            angle: getRandom() * doublePI,
            radius: distance,
    ...
    class MovePluginInstance {
        ...
        particleUpdate(particle, delta) {
            ...
            const container = this.#container, slowFactor = getProximitySpeedFactor(particle), reduceFactor = container.retina.reduceFactor, baseSpeed = particle.retina.moveSpeed, moveDrift = particle.retina.moveDrift, maxSize = particle.size.max, sizeFactor = moveOptions.size ? particle.getRadius() / maxSize : defaultSizeFactor, deltaFactor = delta.factor || defaultDeltaFactor, moveSpeed = baseSpeed * sizeFactor * slowFactor * deltaFactor * half$1, maxSpeed = particle.retina.maxSpeed;
    ...
    class LinkInstance {
        ...
        #drawTriangles(options, p1, link, p1Destinations, pos1, pos2, context) {
            ...
            for (const vertex of p2Links) {
                ...
                const opacityTriangle = triangleOptions.opacity ?? (link.opacity + vertex.opacity) * half$1, colorTriangle = rangeColorToRgb(this.#pluginManager, triangleOptions.color) ?? link.color;
    ...
    const squareExp = 2, absorbFactor = 0.033, minOrbitLength = 0, minRadius = 0, minMass = 0, minAngle = 0, maxAngle = doublePI, maxDegreeAngle = 360, angleIncrementFactor = identity$3 / maxDegreeAngle, minVelocity = 0, defaultLifeDelay$1 = 0, minLifeCount$1 = 0, defaultSpawnDelay$1 = 0, defaultLifeCount$1 = -1;
    class AbsorberInstance {
        ...
        #updateParticlePosition = (particle, delta, v) => {
            ...
            if (this.options.orbits) {
                ...
                const maxSize = particle.size.max, sizeFactor = particle.options.move.size ? particle.getRadius() / maxSize : identity$3, deltaFactor = delta.factor || identity$3, baseSpeed = particle.retina.moveSpeed, moveSpeed = baseSpeed * sizeFactor * deltaFactor * half$1;
        ...
        #calcPosition() {
            ...
            if (this.options.domId) {
                ...
                if (element) {
                    ...
                    return {
                        x: (elRect.x + elRect.width * half$1) * pxRatio,
                        y: (elRect.y + elRect.height * half$1) * pxRatio,
                    };
                }
            }
    ...
    exports.minVelocity = minVelocity$7;
    exports.minZ = minZ;
